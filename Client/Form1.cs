...
namespace Client
{
    public partial class NATIONALWS : Form
    {
        public NATIONALWS()
        {
            InitializeComponent();
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var client = new ServiceReference1.Lab_091SoapClient();
        ...
        private async void btn_GetPresidentName_Click(object sender, EventArgs e)
        {
            try
            {
                var client = new ServiceReference1.Lab_091SoapClient();
                ...
                var reponse = await client.GetPresidentNameAsync(countryCode);
                var president = reponse.Body.GetPresidentNameResult;
                if (president != null && president.Length > 0)
                {
                    MessageBox.Show($"President is: {president}","OK" , MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("No match found!","", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
